...
{
	if(gpio_request(ts_global_reset_pin,NULL) != 0){
		gpio_free(ts_global_reset_pin);
		printk("zhongchu_init_platform_hw gpio_request error\n");
		return -EIO;
	}
	gpio_direction_output(ts_global_reset_pin, 0);
	gpio_set_value(ts_global_reset_pin,1);
...
{
	gpio_direction_output(ts_global_reset_pin, GPIOF_OUT_INIT_LOW);
	gpio_set_value(ts_global_reset_pin,GPIOF_OUT_INIT_LOW);
	return 0;
}

static int gslX680_shutdown_high(void)
{
	gpio_direction_output(ts_global_reset_pin, GPIOF_OUT_INIT_HIGH);
	gpio_set_value(ts_global_reset_pin,GPIOF_OUT_INIT_HIGH);
	return 0;
}
...
static int gsl_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!ts)
		return -ENOMEM;
	printk("==kzalloc succes=\n");

	ts->irq_pin = of_get_named_gpio_flags(np, "touch-gpio", 0, (enum of_gpio_flags *)&irq_flags);
	ts_global_reset_pin = of_get_named_gpio_flags(np, "reset-gpio", 0, &rst_flags);
	ts->client = client;
...
static struct of_device_id gsl_ts_dt_ids[] = {
