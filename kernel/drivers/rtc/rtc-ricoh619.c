...
static int ricoh619_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	int err;
	int cent_flag;
	unsigned long  alarm_sec, now_sec, change_sec, diff_sec;
	struct rtc_time *tm;


	tm = kzalloc(sizeof(struct tm), GFP_KERNEL);
        if (!tm)
                return -ENOMEM;
	//printk("before %d %d %d %d %d %d \n", alrm->time.tm_sec, alrm->time.tm_min, alrm->time.tm_hour, alrm->time.tm_mday, alrm->time.tm_mon, alrm->time.tm_year);
	err = ricoh619_rtc_read_time(dev, tm);
	if (err)
		return err;
	rtc_tm_to_time(tm, &now_sec);
	//rtc_tm_to_time(&alrm->time, &alarm_sec);
	//diff_sec = alarm_sec - now_sec;
        //printk("### diff_sec= %d\n", diff_sec);
        change_sec = now_sec + 60 * 60 * 2;
        rtc_time_to_tm(change_sec, &alrm->time);
	//printk("after %d %d %d %d %d %d \n", alrm->time.tm_sec, alrm->time.tm_min, alrm->time.tm_hour, alrm->time.tm_mday, alrm->time.tm_mon, alrm->time.tm_year);
