...
static void ricoh619_device_shutdown(struct i2c_client *client)
{
	...
	//add by softwin for MIPI reset power control
	ret = ricoh619_read(ricoh619->dev, 0x46, &val);
	printk("###### 0x46 = 0x%x  ######\n", val);
	ret = ricoh619_clr_bits(ricoh619->dev, 0x46, (0x1<<5)); //disable ldo6
	if (ret < 0) {
		printk("###### CLR LDO6 FAIL  ######\n");
		mdelay(1);
		ret = ricoh619_clr_bits(ricoh619->dev, 0x46, (0x1 <<5)); //disable ldo6
	}
	ret = ricoh619_read(ricoh619->dev, 0x46, &val);
	printk("###### 0x46 = 0x%x(after disable)  ######\n", val);
	//end
