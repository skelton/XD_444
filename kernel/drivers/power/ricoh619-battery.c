				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		if(info->capacity < 100 && (info->status == 1))
		{
			gpio_direction_output(info->charge_led, info->charge_led_flag);
		} else {
			gpio_direction_output(info->charge_led, !info->charge_led_flag);
		}
...
static int ricoh619_battery_probe(struct platform_device *pdev)
{
	...
	info->alarm_vol_mv = pdata->alarm_vol_mv;
        ...
        info->charge_led = of_get_named_gpio_flags(node, "charge_led", 0, &flags);
        if(!gpio_is_valid(info->charge_led)){
                dev_err(&pdev->dev, "invalid charge_led gpio%d\n", info->charge_led);
        }

        ret = devm_gpio_request(&pdev->dev, info->charge_led, "charge_led_gpio");
        if (ret) {
                dev_err(&pdev->dev, "failed to request GPIO%d for charge_led\n", info->charge_led);
                return -EINVAL;
        }
        info->charge_led_flag = (flags == GPIO_ACTIVE_HIGH)? 1:0;
